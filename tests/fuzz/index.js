...
module.exports = (ctx) => {
  ...
  it('lock 1000 tokens (mainnet) and kill nodes', async () => {
    ...
    const depositTx = await ctx.providers.main.contracts.TimeHolder.methods.deposit(timeAddress, 1000).send({
      from: ctx.userWallet.address,
      gas: 5700000
    });
    ...
    const lockTx = await ctx.providers.main.contracts.TimeHolder.methods.lock(timeAddress, 1000).send({
      from: ctx.userWallet.address,
      gas: 5700000
    });
  ...
  it('restart node with another oracle', async () => {

    ctx.sidechainPid.kill();

    ctx.thirdAccount = web3.eth.accounts.create();

    let keys = {
      ORACLE_PRIVATE_KEY: ctx.thirdAccount.privateKey,
      SIDECHAIN_ORACLE_PRIVATE_KEY: ctx.thirdAccount.privateKey
    };

    ctx.sidechainPid = spawn('node', ['index.js'], {env: _.merge({}, process.env, keys), stdio: 'ignore'});
...
/*
  it('locking 1000 tokens again (mainnet)', async () => {
